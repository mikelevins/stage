;;;; ***********************************************************************
;;;;
;;;; Name:          repl-widget.lisp
;;;; Project:       stage
;;;; Purpose:       the QTools repl window
;;;; Author:        mikel evins
...
(defun output-prefix (repl)
  (output-colored repl "darkred" "~a&gt;" (escape (package-name *package*)))
  (output repl "&nbsp;")
  (setf (input-begin repl) (cursor repl)))

(defun output-comment (repl format-string &rest args)
  (output-colored repl "darkolivegreen" "; ~a<br />" (apply #'format NIL format-string args)))

(defun output-result (repl values)
  (if values
      (dolist (value values)
        (output-colored repl "darkblue" "~a<br />" (escape (write-to-string value))))
      (output-comment repl "No values.")))

(defun output-error (repl error)
  (output-comment repl "<span style=\"color:darkred;\">Error:</span> ~a" (escape (princ-to-string error)))
...
(defmethod stream-finish-output ((stream repl-output-stream))
  (let ((string (get-output-stream-string (buffer stream))))
    (output-colored (repl stream) "maroon" "~a" (cl-ppcre:regex-replace-all "\\n" (escape string) "<br />")))
  (clear-output stream))
...
(define-subwidget (evaluator layout) (q+:make-qvboxlayout evaluator)
  (setf (q+:window-title evaluator) "Stage Listener")
